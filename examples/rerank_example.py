...
from lightrag.reranker import RerankModel,custom_rerank
...
async def my_rerank_func(query: str, documents: list, top_n: int = None, **kwargs):
    ...
    return await custom_rerank(
        ...
        documents=documents,
        model="rerank-v3.5",
        base_url="hhttps://api.cohere.ai/v1/rerank",
        api_key="",
        top_n=top_n or 10,
        **kwargs,
    )
...
async def create_rag_with_rerank():
    ...
    test_embedding = await openai_embed(["test"])
...
async def test_rerank_with_different_settings():
    ...
    print(f"   Preview: {result_with_rerank[:10000]}...")
    ...
    print(f"   Preview: {result_without_rerank[:10000]}...")
    ...
    print(f"   Preview: {result_default[:1000]}...")
